...
void Display(){
	...
	uint8_t *buf = malloc(1000*read*sizeof(uint16_t)*2);
	_read(fd,buf,2000*read*sizeof(uint16_t));
	uint16_t *points = malloc(1000*read*sizeof(uint16_t)*2);
	bzero(points,2000*sizeof(*points));
	for(int i = 0; i < 500*read;i++){
		points[i] = buf[i*2] << 8 | buf[i*2+1];
	}
	if(log){
		fwrite(points,1,500*sizeof(uint32_t),log);
		fclose(log);
	}
//	for(int i = 0; i < 500;i++){
//		if(!(i % (int)read == 0)){
